...
<div class="content car-rental-list-item">
    ...
    <div class="col3 item-price">
        ...
        <span title="<?php
        if($item['tax_percentage'] > 0):
            print($item['unit_per_period_print']['discounted_total'].' '.$objLang->getText('NRS_WITHOUT_TAX_TEXT').' + ');
            print($item['unit_per_period_print']['discounted_tax_amount'].' '.$objLang->getText('NRS_TAX_TEXT').' = ');
            print($item['unit_per_period_print']['discounted_total_with_tax']);
        endif;
        ?>" class="price-per-period" style="cursor:pointer">
            <?php print($item['unit_per_period_print']['discounted_total_dynamic'].' / '.$item['time_extension_long_print']); ?>
        </span>
    </div>
